...
class Part3TopicTypeSerializer(serializers.ModelSerializer):
    class Meta:
        model = TopicType
        fields = ("id", "name")


class Part3VocabularySerializer(serializers.ModelSerializer):
    class Meta:
        model = Vocabulary
        fields = '__all__'


class Part3IdeaSerializer(serializers.ModelSerializer):
    class Meta:
        model = Idea
        fields = '__all__'


class Part3AnswerSerializer(serializers.ModelSerializer):
...
class Part3QuestionSerializer(serializers.ModelSerializer):
    ...
    def get_answers(self, obj):
        ...
        return Part3AnswerSerializer(qs, many=True).data

    def get_ideas(self, obj):
        qs = obj.part_3_ideas.all()
        return Part3IdeaSerializer(qs, many=True).data

    def get_vocabularies(self, obj):
        qs = obj.part_3_vocabularies.all()
        return Part3VocabularySerializer(qs, many=True).data


class Part3TopicSerializer(serializers.ModelSerializer):
    questions = serializers.SerializerMethodField()
    topic_type = Part3TopicTypeSerializer(many=True, read_only=True)
    ...
    def get_questions(self, obj):
        qs = obj.part_3_questions.all()
        return Part3QuestionSerializer(qs, many=True).data
